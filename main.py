...
def cmd_predict(args):
    ...
    head_path = Path(args.head) if args.head else ROOT_DIR / "models" / "pe-core-l__linear__allext_nodalle3_e1.pt"
    if not head_path.exists():
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_predict.add_argument(
        "--head", default=None,
        help="Head checkpoint path (default: models/pe-core-l__linear__allext_nodalle3_e1.pt).",
    )
    p_predict.add_argument("--threshold", type=float, default=None,
                           help="Decision boundary for the flagged/not-flagged summary line "
                                "(default: predict.DECISION_THRESHOLD = 0.955, chosen on a held-out "
