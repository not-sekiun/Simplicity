...
#     nosd3_e1           0.990         0.0180         0.9692
#     alltransforms_e1   0.985         0.0248         0.9768
#     allsev_e1          0.980         0.0215         0.9797   <-- previous
#     allext_nodalle3_e1 0.955         0.0045         0.9822   <-- SHIPPING
#
...
#
# allext_nodalle3_e1 adds wildrf_real (1,555 real Reddit/X/Facebook photos --
# same source family as the WildRF threshold tier, disjoint images) to
# allsev_e1's pool, trained with --with-chains (11 views) rather than
# --all-severities (19) because wildrf_real's cache only covers the with-chains
# view set. Also NOT strict dominance, and less rigorously vetted than the
# allsev_e1 row above (no CI, no matched-rate comparison, no FINDINGS entry
# yet): wildrf_test clean/17-view-mean AUC improves (.9969/.9875 -> .9987/.9922)
# and so does dalle3_holdout worst-view recall (.7220 -> .7367, still unseen by
# both heads), but ood 17-view-mean AUC REGRESSES (.9724 -> .9690) and
# demo_val's ties within noise. Net read: real-side data (more WildRF-like
# reals) traded a bit of transform-severity coverage for a lot of WildRF FPR --
# consistent with this project's repeated finding that real-side data moves the
# numbers more than synthetic-severity coverage does, but re-derive and re-check
# this trade before trusting it the way allsev_e1's row above has been checked.
#
# At threshold 0.5 the WildRF tier gives FPR 0.1875 / TPR 0.9949 (allsev_e1), so
# cutting to ~0.98 is a large reduction in false positives for a small amount of
# recall. That trade is right for this product: telling someone their own
# photograph is AI-generated costs far more than missing one AI image among many.
DECISION_THRESHOLD = 0.955
